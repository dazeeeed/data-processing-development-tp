...
object ClimateService {
	...
	def isClimateRelated(description: String): Boolean = {
		if(description.toLowerCase().contains("global warming") ||
			description.toLowerCase().contains("ipcc") ||
			description.toLowerCase().contains("climate change")){
			true
		} else {
			false
		}
	...
	def parseRawData(list: List[(Int, Int, Double)]) : List[Option[CO2Record]] = {
		list.map { record =>
			val ppm = record._3
			val co2record = CO2Record(record._1, record._2, ppm)
	 ...
	 *
	 * @param list of optional aco2records
	 * @return a list
	 */
	def filterDecemberData(list: List[Option[CO2Record]]) : List[CO2Record] = {
		// Flatten the list (remove the None so only CO2Records are there) and filter by month
	 ...
	 * get min and max ppm in the record list
	 *
	 * @param list of co2records
	 * @return (minPpm, maxPpm)
	 */
	def getMinMax(list: List[CO2Record]) : (Double, Double) = {
		val minPpm = list.map(record => record.ppm).min
		val maxPpm = list.map(record => record.ppm).max

		return (minPpm, maxPpm)
	}

	/**
	 * get min and max ppm in the record list for a specified year
	 *
	 * @param list, year
	 * @return (minPpm, maxPpm) for a specified year
	 */
	def getMinMaxByYear(list: List[CO2Record], year: Int) : (Double, Double) = {
		val recordsInYear = list.filter(_.year == year)

		ClimateService.getMinMax(recordsInYear)
	}
